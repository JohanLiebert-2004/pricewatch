...
| P15 | Mobile barcode scanner and black preview | Codex | **Implemented 22 July; commit/deploy blocked by sandbox** | `web/search.html`, `web/style.css`, `web/sw.js`. The previously shipped ZXing fallback now has explicit loading/starting/ready/error states; partial native `BarcodeDetector` implementations fall back before camera permission; the live `<video>` stays hidden behind a neutral camera placeholder until frames are available; close/error clears tracks, `srcObject`, and ARIA expansion state; service-worker cache bumped to v7 without eagerly downloading the 336 KB fallback. Verified: deterministic Node simulations pass for both native and Safari/ZXing scanner lifecycles (open/ready/close, track stop, frame clear, ARIA state); all 5 unit tests pass; Python compile passes; workflow YAML/HTML/inline JS/service-worker syntax parse; and `git diff --check` passes. No physical-phone camera test or production deployment in this restricted session: `.git` is read-only (`index.lock` denied), so the focused commit/push required by protocol cannot be created here. |
