...
| P21 | Fix local sweeps broken by P19's port closure; move them onto a new always-on Ubuntu laptop | Claude | **In progress 21 July** | **Found:** closing public 5432 (P19) also silently broke the Windows-owned Big W and Chemist Warehouse local sweeps - both had been failing with `psycopg.errors.ConnectionTimeout` since ~07:32/09:02 UTC, unnoticed until checked. **Fixed:** new `scripts/db_tunnel.py` opens a private SSH local-forward through the web VM's existing `ci-tunnel` account (`restrict,port-forwarding,permitopen="10.42.1.9:5432"`, no shell/TTY) before each sweep, rewrites `DATABASE_URL` to the tunnel endpoint for that process only, and fails closed (skips the sweep, no fallback to a direct connection) on any tunnel error. Host key is pinned in `scripts/tunnel_known_hosts`, not `StrictHostKeyChecking=no`. Each consumer has its own dedicated forwarding-only key added to `ci-tunnel` - GitHub Actions' existing key, a new one for this Windows PC (`pricewatch_tunnel_local_ed25519`), and a new one for the Ubuntu laptop (`pricewatch_tunnel_ubuntu_ed25519`) - none shared. **Live bug found and fixed while testing:** the first Chemist Warehouse retest raced - sshd logged the tunnel session opening then closing one second later, but the bare "local port is open" readiness probe had already declared success, so the crawl subprocess then hit a downstream `ConnectionTimeout` against a tunnel that no longer existed. `db_tunnel.py` now re-confirms the SSH process is still alive after a short settle period before trusting it, and allocates a fresh OS-assigned local port per call instead of a shared fixed one. **Verified live:** Big W through the tunnel from this Windows PC fetched 864 listings, wrote 66 snapshots, wrote a fresh `bigw_local_heartbeat` - confirmed against production directly. **New Ubuntu laptop** (`192.168.0.33`, 8 cores/14GB/140GB free, `johan` user, `~/pricewatch` clone, Python venv, `systemd --user` timers with `loginctl enable-linger` so they survive logout/reboot without needing `sudo`, which this account doesn't have) is set up as the intended new home for both sweeps. Once confirmed, disable the Windows Task Scheduler tasks (`Dealwatch BigW sweep`, `Dealwatch ChemistWarehouse sweep`) so only one lane per retailer runs, matching the one-crawler-per-retailer rule - keep the Windows scripts working as a dormant fallback rather than deleting them, same convention as the OCI host fallback. Also cleaned up the stale host-level iptables rule on the DB VM (was `0.0.0.0/0:5432 ACCEPT`, now scoped to `10.42.1.0/24` only, matching what the OCI Security List already enforces - this rule never mattered for internet traffic since the Security List blocks it first, but left unscoped it was misleading and didn't reflect intent).
  **Status as of this writing - Big W confirmed, Chemist Warehouse still not proven.** Big W: fully verified end-to-end from this Windows PC (864 listings, 66 snapshots, fresh `bigw_local_heartbeat` confirmed directly against production). Chemist Warehouse (~70min/run, both Windows and Ubuntu) has failed on **every** retest so far, but for two different, now-understood reasons, neither of which is a flaw in the tunnel design itself: (1) the tunnel-liveness race described above, now fixed in `db_tunnel.py`; (2) both machines' retests failed simultaneously (`psycopg.OperationalError: the connection is lost`, ~29min into two separate ~70min runs, timestamps 2 seconds apart) because **the owner restarted their home modem mid-run** - confirmed by the owner directly, not inferred - which drops both machines' internet at once since they share it. Retests were relaunched after the modem settled and were still in progress (~30min into a new run each) at the time of this note. **Do not mark Chemist Warehouse fixed until a run actually completes and writes `chemistwarehouse_local_heartbeat`** - the external watchdog (P22 below) independently confirmed this heartbeat is still 3 days stale as of its first run, which is accurate, not a false positive. |
