...
| 18 July Myer crawl_batch revert (`<pending>`) | The `2400` batch size above was **actively counterproductive**, not just slow: actual per-item time ran 2-3x the assumed 1.5s delay floor, so each myer job took 1.5-2h instead of the estimated 60min. Every hourly cron tick queued a new run for the `crawl-myer` concurrency group, which cancelled the still-running previous run before it finished. Confirmed via `gh run view` on the last 4 consecutive scheduled runs (29609570124, 29613191054, 29616380989, 29618999424): **every single one was cancelled**, none completed, spanning ~20:00-23:51 UTC on 17 July. This also correlates with the `detect` job failing on 2 of those runs with `psycopg.errors.QueryCanceled: canceling statement due to statement timeout` while inserting into `deals` - likely lock/connection contention from the pile-up of overlapping cancelled-but-still-cleaning-up myer jobs; **not independently root-caused, just observed as concurrent**. Reverted `crawl_batch` 2400 -> 1000 (the last value confirmed to reliably complete inside an hour) rather than guessing a new number. **Watch the next 2-3 scheduled runs**: confirm myer completes (not cancelled) and `detect` stops throwing `statement timeout` - if `detect` still fails after myer stabilises, the timeout has a separate cause and needs its own investigation (check Supabase's configured `statement_timeout`, or whether the `deals` insert/upsert itself has grown slow from table bloat). |
