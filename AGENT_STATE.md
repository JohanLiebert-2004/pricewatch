...
| P24 | Fix Ubuntu sweep timers: Big W not running at all, Chemist not durable across reboot | Claude | In progress | `~/.config/systemd/user/pricewatch-bigw.timer` + `pricewatch-chemistwarehouse.timer` on the Ubuntu laptop only (no repo files). Live cross-check 22 July ~01:32 UTC (read-only kv/heartbeat query through the tunnel, plus SSH to the laptop) found `46c2f63`'s "migration to Ubuntu complete" claim only actually holds for Chemist Warehouse. Chemist: heartbeat 35min old, timer active but `disabled` (won't survive reboot/logout). Big W: heartbeat 12h stale (`2026-07-21T13:31:44Z`), its Ubuntu timer is both `disabled` and `inactive` (`Trigger: n/a` - not scheduled at all), and both Windows tasks are confirmed `Disabled` - no lane is running it. CI's own cadence gate treats <24h as fresh, so it won't fall back on its own for up to 24h. Fixing: `systemctl --user enable --now` both timers on the laptop so they're active and durable, then trigger one immediate Big W run to prove it works and refresh the stale heartbeat rather than waiting up to 3h for its own schedule. |
