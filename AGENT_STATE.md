...
- **Two home machines now run production sweeps/services, both outside OCI
  ...
  Ubuntu laptop currently runs: Big W sweep (3h), Chemist Warehouse sweep
  (2h), an independent local DB backup (daily), and the external watchdog
  (30min). Windows PC still runs the same two sweeps in parallel as of
  this writing - disable its Task Scheduler tasks once Ubuntu's Chemist
  Warehouse lane is confirmed (see P21), don't just assume it already
  happened.
