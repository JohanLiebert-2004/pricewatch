...
| P20 | Finish post-P19 crawler production proof | Claude | **Pending 21 July** | The first corrected Kmart process began at 09:44 UTC on deployed `5abd497` and was still active at 10:23; the 09:44 error in the log belongs to the pre-deploy 08:23 process. Confirm the new run exits 0 and writes `kmart_vm_heartbeat`. Watch enrichment run `29820953050` (final tunnel retry, max-parallel 3, action-v7 revision); Big W, Target, and Officeworks were green and the three long jobs had successful tunnels at handoff. After the heartbeat, verify one full hourly workflow where Kmart skips and detect succeeds. If the pre-first-heartbeat overlap repeats, add a start-lease marker; do not mask failures or reopen port 5432. Full commands/evidence are in the final README handover. |
