...
- Decision: the user chose **Ultimate Web Scraper** for Target and confirmed
  they do not have Commission Factory. Do not route this task back to
  Commission Factory unless the user changes that decision.
- Done: implemented a feed-first Target catalogue path that accepts stable UWS
...
- Verified: 13 unit tests pass; Python compilation, both workflow YAML parses,
  ...
  changed.
- Deferred state: the Ultimate Web Scraper MCP connector was not available to
  this Codex session. No UWS automation, cloud schedule, stable export, webhook,
  GitHub secret, workflow dispatch, database write, or credit spend occurred.
  The current code is a pull importer; a UWS webhook receiver is **not** built.
- When the user resumes: connect UWS through Dashboard -> Integrations -> MCP,
  test only three Target product-detail URLs first, show the credit estimate
  before running, inspect the resulting columns, and then choose either a
  stable HTTPS export for `TARGET_PRODUCT_FEED_URL` or design an authenticated
  webhook receiver. Only after that should `refresh-and-detect` be dispatched
  and Target's non-zero count/fresh `retailer_freshness` row be verified.
