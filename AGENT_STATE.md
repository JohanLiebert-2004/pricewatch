...
- **19 July — DONE (Claude): site cut over to self-hosted Postgres +
  ...
  **21 July, ~05:48 UTC - a fourth consecutive ~4h retry window (relaunched
  per explicit owner direction to "keep grinding" after the third) also
  exhausted all 48 attempts, still exclusively "out of host capacity" on
  `oci_core_instance.pricewatch_db` (main.tf:190) every single attempt -
  no new/different error type.** `ap-sydney-1` A1.Flex capacity has now
  shown zero availability across 4 separate ~4h windows spanning 19-21
  July. No retry loop is running as of this note. Given the pattern is
  now well-established (immediate back-to-back bursts keep re-hitting the
  same shortage, per the spacing suggestion above that hasn't been tried
  yet), the next person picking this up should ask the owner before just
  relaunching another identical burst - either try the spaced-out cadence,
  a different availability domain, or accept `pricewatch_db_x86` as
  long-term rather than interim.
...
| P15 | Mobile barcode scanner and black preview | Codex, committed/deployed by Claude | **Done 22 July** | `web/search.html`, `web/style.css`, `web/sw.js`. The previously shipped ZXing fallback now has explicit starting/loading/scanning/error states; the live `<video>` stays hidden behind a neutral camera placeholder until `data-state="scanning"`; close/error now also pauses the video and clears `srcObject` (not just hides the container), plus ARIA expansion state; guards against double-starting the scanner on a repeat click (previously could spin up two camera streams/ZXing readers); service-worker cache bumped to v7. Codex's sandbox had a read-only `.git` and could not commit/push. Independently re-verified by Claude before committing (`2d90b0b`): `git diff --check` clean, inline `<script>`/`sw.js` parse, `style.css` braces balance, all 5 existing unit tests pass, and a Node DOM shim confirms `stopScanner()` resets hidden/state/aria-expanded/`srcObject` and a repeat click while already open doesn't restart the scanner. Deployed: `vercel --prod --yes` for `web/`. Verified live: `/sw.js` serves `dealwatch-static-v7`, `/search.html` and `/style.css` contain the new `.scan-preview`/`.scan-placeholder` markup and rules. **Still not done: no physical-phone camera test in any session so far** - the actual "black preview" symptom has only been reasoned about and simulated, never watched fixed on a real device. |
| P16 | Product-page and sitemap SEO hardening | Codex, deployed by Claude | **Done 21 July** | `services/preview_app.py`, `views.sql`, `web/landing.html`, `web/sitemap-pages.xml`, `web/vercel.json`, `tests/test_preview_seo.py`. Independently re-verified before committing (`a33bffb`): all 4 focused tests pass locally, plus `ast`/`json`/`xml` syntax checks. Deployed: `views.sql` re-applied against `pricewatch-db-x86` as the `postgres` superuser, `git pull` + `systemctl restart pricewatch-web.service` on 159.13.59.184, `vercel --prod --yes` for `web/`. Verified live with real requests (not a bare curl - see the empty-Bearer lesson above): a real product page (`/p/officeworks/SECW15WWNK`) returns exactly one `<meta name="description">`, real SSR title/breadcrumb content, `Cache-Control: public, max-age=300, s-maxage=300, stale-while-revalidate=60`, and a single JSON-LD `@graph` with `Product` (`InStock`, since the row was fresh) + `BreadcrumbList`; `/product.html` (the legacy client-rendered shell) returns `X-Robots-Tag: noindex, follow`; a category landing page (`/deals/tech`) shows a real data-backed insight line and the new discovery footer; `/sitemap-products-1.xml` returns only today's URLs. |
| P17 | Refresh the root project README | Codex | **Done 21 July** | `README.md` now documents the current Dealwatch product, 13-retailer registry, Vercel/PostgREST/OCI architecture, SQLite and PostgreSQL workflows, environment variables, repository map, deployment pointers, responsible crawling policy, and verified CLI/test commands. `git diff --check` passed and all 4 focused unit tests passed. |
| P18 | Live website and OCI handover audit | Codex | **Done 21 July** | `README.md` and `AGENT_STATE.md` only. Verified live public routes/API/SEO, ran mobile Lighthouse (92/96/100/100), checked recent Actions runs, validated Terraform/state and raw cloud-init YAML, and inspected both OCI hosts read-only over SSH. README now records healthy services, uneven freshness, invalid cloud-init, unreproducible DB provisioning, crawler overlap/overruns, public/shared network rules, hardcoded endpoints, and prioritized Claude actions. No production state changed and no secrets were read or printed. |
| P19 | Resolve remaining live-site and OCI handover findings | Codex | **Implemented 21 July; live follow-up handed to Claude** | Commits `bccb32c` through `9056b64`: explicit PostgreSQL input casts plus Kmart brand normalisation and regression tests; hourly refresh/detect split from three-hour enrichment; retrying private CI DB tunnel with forwarding-only identity and VCN-only PostgreSQL; action-v7 upgrade; homepage bootstrap RPC, 24-card paging, same-origin images, dimensions, system fonts, and contrast fixes; role-specific valid cloud-init, reproducible DB bootstrap, ARM opt-in, stable DB-private-IP variable, and current OCI/deployment docs. Five tests pass, rendered cloud-init parses, Terraform validates and live-plans **No changes**, production routes/RPC return 200, and final mobile Lighthouse is 96/100/100/100. |
| P20 | Finish post-P19 crawler production proof | Claude | **Done 21 July** | All three verification steps confirmed. (1) The 09:44 UTC Kmart process (deployed `5abd497`) exited 0 and wrote `kmart_vm_heartbeat` at **10:53:13Z** - confirmed directly against production: `kmart_vm_heartbeat` present, 84,614 Kmart products (up from 82,093), freshest row 11:47:46Z. (2) Enrichment run `29820953050`: **10/10 jobs finished green** (Big W, Target, Officeworks, Good Guys, Myer, Supercheap, Chemist Warehouse, QBD, Booktopia, IKEA), zero failures. (3) Hourly `refresh-and-detect` run `29824457827` (11:01:06Z, within the fresh heartbeat's window): the Kmart job's `Refresh kmart` step shows **`skipped`** - confirming the cadence gate correctly deferred to the VM heartbeat - every other refresh job succeeded, and `detect` succeeded. |
| P21 | Fix local sweeps broken by P19's port closure; move them onto a new always-on Ubuntu laptop | Claude | **In progress 21 July** | **Found:** closing public 5432 (P19) also silently broke the Windows-owned Big W and Chemist Warehouse local sweeps - both had been failing with `psycopg.errors.ConnectionTimeout` since ~07:32/09:02 UTC, unnoticed until checked. **Fixed:** new `scripts/db_tunnel.py` opens a private SSH local-forward through the web VM's existing `ci-tunnel` account (`restrict,port-forwarding,permitopen="10.42.1.9:5432"`, no shell/TTY) before each sweep, rewrites `DATABASE_URL` to the tunnel endpoint for that process only, and fails closed (skips the sweep, no fallback to a direct connection) on any tunnel error. Host key is pinned in `scripts/tunnel_known_hosts`, not `StrictHostKeyChecking=no`. Each consumer has its own dedicated forwarding-only key added to `ci-tunnel` - GitHub Actions' existing key, a new one for this Windows PC (`pricewatch_tunnel_local_ed25519`), and a new one for the Ubuntu laptop (`pricewatch_tunnel_ubuntu_ed25519`) - none shared. **Live bug found and fixed while testing:** the first Chemist Warehouse retest raced - sshd logged the tunnel session opening then closing one second later, but the bare "local port is open" readiness probe had already declared success, so the crawl subprocess then hit a downstream `ConnectionTimeout` against a tunnel that no longer existed. `db_tunnel.py` now re-confirms the SSH process is still alive after a short settle period before trusting it, and allocates a fresh OS-assigned local port per call instead of a shared fixed one. **Verified live:** Big W through the tunnel from this Windows PC fetched 864 listings, wrote 66 snapshots, wrote a fresh `bigw_local_heartbeat` - confirmed against production directly. **New Ubuntu laptop** (`192.168.0.33`, 8 cores/14GB/140GB free, `johan` user, `~/pricewatch` clone, Python venv, `systemd --user` timers with `loginctl enable-linger` so they survive logout/reboot without needing `sudo`, which this account doesn't have) is set up as the intended new home for both sweeps. Once confirmed, disable the Windows Task Scheduler tasks (`Dealwatch BigW sweep`, `Dealwatch ChemistWarehouse sweep`) so only one lane per retailer runs, matching the one-crawler-per-retailer rule - keep the Windows scripts working as a dormant fallback rather than deleting them, same convention as the OCI host fallback. Also cleaned up the stale host-level iptables rule on the DB VM (was `0.0.0.0/0:5432 ACCEPT`, now scoped to `10.42.1.0/24` only, matching what the OCI Security List already enforces - this rule never mattered for internet traffic since the Security List blocks it first, but left unscoped it was misleading and didn't reflect intent).
  **Status as of this writing - Big W confirmed, Chemist Warehouse fixed by shrinking the batch, not yet proven by a live success.** Big W: fully verified end-to-end from this Windows PC (864 listings, 66 snapshots, fresh `bigw_local_heartbeat` confirmed directly against production). Chemist Warehouse's original 400-item/~70min batch failed **four times in a row**, every time with a different symptom, all consistent with one root cause (a single SSH tunnel over home internet not surviving an hour-plus): (1) the tunnel-liveness race, fixed in `db_tunnel.py`; (2) both machines' retests died within 2 seconds of each other because **the owner restarted their home modem mid-run** (confirmed directly, not inferred - drops both machines' internet at once since they share it); (3) a `psycopg.OperationalError: the connection is lost`; (4) a final attempt on the old code simply hit the script's own 5400s subprocess timeout without ever finishing - confirmed server-side too (`could not receive data from client: Connection reset by peer`, logged from the web VM's private IP, the tunnel's relay point). Rather than keep patching individual symptoms, cut what has to survive the tunnel: `CRAWL_BATCH` 400 -> 100 (~70min -> ~17-18min/run), timer cadence 2h -> 30min on **both** machines (same ~200 items/hour throughput). Deployed and pulled on both Windows and the Ubuntu laptop.
  **Now proven on both machines, migration to Ubuntu complete.** Windows: clean run at 14:25:01Z, 92/100 stored, heartbeat written. Ubuntu: clean run at 23:07:39Z, 87/100 stored, heartbeat written, and its next 30min cycle was already under way at the time of this note - the 30min/100-item cadence is holding steady on its own. The P22 watchdog independently confirmed the recovery too (`all checks OK` at 23:00:48Z, no manual check needed - this is exactly the failure mode it exists to catch and it worked as designed). Both machines were briefly running Chemist Warehouse **at the same time** (23:07:39 on Ubuntu overlapping 23:16:34 on Windows) - a real one-crawler-per-retailer violation, not just a theoretical risk. Per the original plan, **both Windows Task Scheduler tasks (`Dealwatch BigW sweep`, `Dealwatch ChemistWarehouse sweep`) are now Disabled** (not deleted - same dormant-fallback convention as the old OCI host) now that Ubuntu is confirmed reliable as the primary lane for both retailers. The already-in-flight Windows Chemist Warehouse run at disable time was left to finish rather than killed; it was the last one that will ever fire automatically. If Ubuntu ever goes offline for an extended period, re-enable the Windows tasks by hand (`Enable-ScheduledTask`) as a stopgap rather than leaving both retailers dark. |
| P22 | Put the new Ubuntu laptop's spare capacity to use: independent backup, relocate the embed service, add an external watchdog | Claude | **Done 21 July** | Owner's explicit request, all three verified live. **(1) Independent local backup** (`scripts/backup_local.py` + `pricewatch-backup-local.timer`, daily 08:00 UTC): the only prior backup lived in OCI Object Storage, *inside the same tenancy as the live DB* - an account-level problem takes out both together. This is a second `pg_dump` on hardware outside OCI entirely, same tunnel as the sweeps, same 30-day retention. Verified live: 429.9MB dump + sha256 checksum landed in `~/pricewatch-backups/` on the laptop. **(2) Relocated the query-embedding service** (`services/embed_server.py` - natural-language search, from the still-unwired `e4721a5` WIP) from the web VM to the DB VM, *not* to the Ubuntu laptop - grepped the whole repo first and confirmed nothing calls it yet (`preview_app.py` has zero references despite its own docstring claiming otherwise), so this was a pure relocation with no caller code to update. Reason: the web VM was down to 159MB available RAM running web+bot+embed together; the DB VM had 420MB available on the same 1GB-class box. Bound to the DB VM's private VCN IP (`10.42.1.9:8301`), reachable from the web VM the same way it already reaches PostgREST (not a new architecture). Required two new private-only firewall holes that didn't exist yet - host iptables (`10.42.1.0/24` only) and a new OCI Security List ingress rule (`terraform apply -target=oci_core_security_list.public`, live plan was clean: 0 added/1 changed/0 destroyed) - both scoped identically to the existing 5432 rule, nothing public. Verified live: HTTP 200 with a real 384-dim vector, called from the web VM over the private network. Web VM's available RAM roughly doubled afterward (159MB -> 325MB). **Also fixed the old template while touching it:** `infra/oci/services/pricewatch-embed.service` referenced a venv path (`/opt/pricewatch/.venv`) that was never the real one (`/opt/pricewatch-venv`) - would have failed to start as originally written; replaced with `pricewatch-embed-db.service` for the new location. **(3) External health watchdog** (`scripts/watchdog.py` + `pricewatch-watchdog.timer`, every 30min): `health_alerts.py` already covers per-retailer staleness, but runs *inside* the CI pipeline it would need to detect failing - confirmed twice this session that this is a real blind spot (P19 silently broke both local sweeps for hours; Kmart's CI lane failed silently for 2+ days). This checks what nothing else can see from outside: GitHub Actions itself still triggering/completing runs (unauthenticated public-repo API, no token needed), each local-sweep heartbeat fresh within its own cadence, both OCI VMs reachable over SSH. Telegram alert on OK->BAD, one recovery notice on BAD->OK, local JSON state file so the monitor doesn't depend on the DB it's partly checking. **Verified live and immediately useful, not just a smoke test**: its first real run correctly caught the still-unresolved Chemist Warehouse heartbeat staleness from P21 and alerted on it - a true positive, not a bug. **Also fixed in passing:** `scripts/backup_postgres_to_oci.sh` was tracked in git as non-executable (`100644`) despite its systemd unit exec'ing it directly with no `bash` prefix - harmless only because someone had manually `chmod +x`'d it on the live box outside of git; any fresh clone/pull would have silently broken the daily OCI backup. Fixed the tracked mode (`git update-index --chmod=+x`), not just the live file. |
...
| P24 | Fix Ubuntu sweep timers: Big W not running at all, Chemist not durable across reboot | Claude | **Done 22 July** | `~/.config/systemd/user/pricewatch-bigw.timer` + `pricewatch-chemistwarehouse.timer` on the Ubuntu laptop only (no repo files). Live cross-check 22 July ~01:32 UTC (read-only kv/heartbeat query through the tunnel, plus SSH to the laptop) found `46c2f63`'s "migration to Ubuntu complete" claim only actually held for Chemist Warehouse. Chemist: heartbeat 35min old, timer active but `disabled` (would not have survived reboot/logout). Big W: heartbeat 12h stale (`2026-07-21T13:31:44Z`), its Ubuntu timer was both `disabled` and `inactive` (`Trigger: n/a` - not scheduled at all), and both Windows tasks were confirmed `Disabled` - no lane was running it at all. CI's own cadence gate treats <24h as fresh, so it would not have fallen back on its own for up to 24h. **Fixed:** `systemctl --user enable --now` both timers on the laptop (confirmed `enabled` + correctly listed with real next-fire times, e.g. Big W next at 12:00 AWST/2h18m out), then triggered one immediate Big W run rather than waiting for its 3h schedule. **Verified live, not just "service exited 0":** confirmed against production directly (same read-only kv query, not trusting the service's own log) - `bigw_local_heartbeat` now `2026-07-22T01:42:00Z` (fresh), bigw fresh-product count rose 276->375 in the same window. The run itself hit Big W's usual intermittent bot-protection block partway through (`HTTP 403 ... bot protection`, same known pattern as prior sessions - not a new issue) but still got 97 real snapshots before the block and wrote its heartbeat; the crawl lane's 0/200 this run will resume from the saved queue cursor next cycle, standard for this scraper. Chemist Warehouse's heartbeat also independently advanced during this check (`2026-07-22T01:33:32Z`), confirming its now-enabled timer is still firing correctly post-enable. |
| P25 | Full infra health check: blocked-retailer alert gap, JB Hi-Fi home sweep, a real detect-job deadlock, OCI kernel reboots | Claude | **Done 22 July** | Owner asked "how are my other services running" - a full check found JB Hi-Fi silently dead for ~24h, a monitoring gap that would have delayed noticing it for up to 36h, an unrelated Postgres deadlock crashing the hourly `detect` job, and both OCI VMs sitting on an already-downloaded but not-yet-active kernel update. All four fixed and verified live. **(1) Blocked-retailer alert gap:** `record_scraper_health()` (`run.py:24`) already existed and was wired into `cmd_crawl` (per-URL queue path) but never into `cmd_refresh` - the fast bulk-listing path every retailer with a listing API uses, including JB Hi-Fi's hourly refresh. A `Blocked` exception there just printed and moved on, so `health_alerts.py`'s dedicated "blocked" fast-path (fires immediately, no age threshold) never triggered; the block instead had to age past the generic 36h default staleness threshold. Wired `cmd_refresh`'s `except Blocked` branch to record "blocked", and a clean sweep to record "ok" (so recovery is detected too) - future blocks on any retailer now surface within about an hour instead of up to 36h later. **(2) JB Hi-Fi home-IP sweep:** its `/products.json` listing feed had been returning HTTP 429 on request #1 of every single hourly GitHub Actions run since ~21 July (confirmed across several consecutive run logs, all identical). A single direct request from a residential IP the same day got a clean 200 with Shopify's complexity-score rate-limit headers present but not tripped - same shape of problem as Big W's Akamai block (shared/datacenter CI IP ranges throttled harder than a home connection). Added `local_jbhifi_sweep.py` (same `scripts/db_tunnel.py` pattern as the other two local sweeps, writes `jbhifi_local_heartbeat` only on a real non-blocked sweep) plus a "JB Hi-Fi cadence gate" step in `crawl.yml` matching the existing Kmart gate (90min threshold). Deployed: `systemctl --user enable --now pricewatch-jbhifi.timer` on the Ubuntu laptop (30min cadence, offset 15min from Chemist Warehouse's own timer to avoid simultaneous tunnel use). **Verified live end-to-end, not just "exited 0":** a manual test run took ~18 minutes (corrected the docstring's initial ~2min guess after measuring it for real) and wrote a real heartbeat - 25,000 listings seen (full window), 16,750 kept, 734 snapshots written. Then manually dispatched the full `crawl.yml` pipeline: the new gate logged `"The home-IP JB Hi-Fi sweep is healthy; skipping CI fallback"` and correctly skipped its own refresh attempt, and `retailer_freshness` (materialized, refreshed by `detect`) showed jbhifi's `last_seen` jump from the stale `2026-07-21T13:52:45Z` to a live `2026-07-22T14:56:44Z` after that run's `detect` job completed. **(3) Unrelated deadlock found while cross-checking a scheduled run:** that same morning's scheduled run (`29923997341`) had its `detect` job crash with `psycopg.errors.DeadlockDetected` inside `categorize.backfill()` - the `embed` job runs concurrently against the same `products` table (already documented and handled with a retry loop in `embed_products.py`), but `backfill()`'s `SELECT` had no `ORDER BY` (non-deterministic per-row `UPDATE` lock order) and no retry at all, so a collision took the whole job down, along with that hour's price-drop alerts. Fixed with `ORDER BY id` plus the same retry-with-backoff shape already proven in `embed_products.py`. Confirmed fixed by the same manual full-pipeline dispatch above: `detect` finished green in 12m27s, no recurrence. **(4) OCI kernel reboots:** both VMs had `unattended-upgrades` already download+install a newer kernel (`6.17.0-1018-oracle` over the running `6.17.0-1011-oracle`) but hadn't rebooted to activate it (`/var/run/reboot-required` set on both, `~16-21` other routine non-security package bumps pending too, left alone). Rebooted web VM first, verified `pricewatch-web.service`/`pricewatch-kmart.timer` active and the site/SSR product page/img proxy all 200 before touching the DB VM; then rebooted the DB VM, verified `postgresql`/`pricewatch-postgrest.service`/`nginx`/`pricewatch-backup.timer` all active, reboot-required flag clear on both, and the live site (including the just-recovered jbhifi freshness data) unaffected throughout. |
| P26 | Restore Target catalogue ingestion through an approved external feed | Codex | **Implemented 1 Aug; waiting for feed URL** | Added a feed-first Target refresh path for approved HTTPS CSV/JSON feeds, strict AUD/positive-price checks, normalized SKU/GTIN/stock/RRP/image/category fields, direct Target URL recovery, duplicate-SKU handling, secret-safe failures, and workflow wiring. The old serial storefront path remains a visible fallback; no Akamai bypass or browser stealth was added. Activation requires setting the private `TARGET_PRODUCT_FEED_URL` GitHub secret from the user's approved Commission Factory/UWS export. Thirteen tests, Python compilation, workflow YAML parsing, and `git diff --check` pass; no live feed was available, so production recovery is not yet claimed. |

## Handoff notes

### 2026-08-01 — Codex
- Done: implemented a feed-first Target catalogue path that accepts approved
  HTTPS CSV or JSON exports and feeds normalized records through the existing
  `ProductRecord`/bulk-upsert pipeline.
- Changed: `scrapers/kmart_group.py`, `tests/test_target_feed.py`, both crawler
  workflows, `README.md`, and this state file.
- Verified: 13 unit tests pass; Python compilation, both workflow YAML parses,
  and `git diff --check` pass. No database rows or external service state were
  changed.
- Next / blocked: create or export the approved Target product feed and set its
  private HTTPS URL as GitHub secret `TARGET_PRODUCT_FEED_URL`; then dispatch
  `refresh-and-detect` and confirm a non-zero Target count plus a fresh
  `retailer_freshness` row before marking recovery complete.
...
- **Two home machines now run production sweeps/services, both outside OCI
  entirely (see P21/P22 for full detail):** the owner's Windows PC (this
  session's machine) and a new always-on Ubuntu laptop (`192.168.0.33`,
  user `johan`, repo at `~/pricewatch`, `systemd --user` timers with
  `loginctl enable-linger`, no `sudo` available). Both reach the DB through
  `scripts/db_tunnel.py` (a private SSH forward through the web VM's
  `ci-tunnel` account) since public 5432 closed in P19 - **never point
  DATABASE_URL at the DB's public IP directly from either machine again,
  it will silently time out.** Each machine has its own dedicated
  forwarding-only SSH key added to `ci-tunnel` (Windows:
  `pricewatch_tunnel_local_ed25519`, Ubuntu: `pricewatch_tunnel_ubuntu_ed25519`,
  GitHub Actions: its own separate one) - if a new machine ever needs DB
  access, give it a new dedicated key the same way, never share one.
  **Scheduler state at the 21 July ~13:20 UTC cross-check:** neither named
  Windows Task Scheduler job was registered. Ubuntu's Chemist service and
  timer were active for an in-progress manual retest, but the timer unit file
  was disabled; Ubuntu's Big W timer was disabled/inactive. Only the local
  backup and watchdog timers were both enabled and active. Re-check live
  state before changing ownership, enable exactly one durable lane per
  retailer, and do not mark Chemist Warehouse fixed until its production
  heartbeat is fresh.
