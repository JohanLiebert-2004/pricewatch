...
| P16 | Product-page and sitemap SEO hardening | Codex | **Ready to deploy** | `services/preview_app.py`, `views.sql`, `web/landing.html`, `web/sitemap-pages.xml`, `web/vercel.json`, `tests/test_preview_seo.py`; product SSR now has one unique description plus visible product content and breadcrumbs, upstream failures return retryable 503s, `InStock` requires a check within 36 hours, product pages use a short CDN cache, the legacy shell gets an exact `X-Robots-Tag`, sitemap rows require a positive price/check within 30 days, all supported retailers are listed, and landing pages include data-backed summaries and crawlable discovery links. Four focused tests plus Python/JS/JSON/XML checks pass. Deployment is blocked in this session: no DB URL/SSH key, GitHub auth is invalid, Vercel is unreachable through the local proxy, and `.git/index.lock` is not writable. |
