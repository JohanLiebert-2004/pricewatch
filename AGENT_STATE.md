...
| P19 | Resolve remaining live-site and OCI handover findings | Codex | **In progress 21 July** | Scope: `db.py`, crawler tests/workflow, `web/`, `infra/oci/`, `README.md`, and this handover. Fix Kmart Postgres type inference, split slow enrichment from hourly detection, apply remaining accessibility/performance changes, make OCI bootstrap role-specific and reproducible without replacing live hosts, remove avoidable hardcoded endpoints, validate locally, then deploy safe in-place changes and record production evidence. |
